...
- Tensor 클래스
  ...
  - 기본적인 연산들은 전부 사용가능 (numpy와 함수명도 거의 동일)
    - Transpose의 경우 permute 라는 함수를 사용
    - max 함수가 (value, index) 튜플형식으로 같이 출력해 argmax가 없음
...
- DataSet
  ...
  - 데이터가 적을떼(메모리에 전부올리기 무리가 없을때)
    ...
    - 데이터가 커 한번에 메모리에 올릴양이 아닌 경우 불러올 준비만 해주고 getitem에서 처리
  - 데이터가 클때
    - init 에서 전부 불러오는게 불가능하므로 패스 설정같은 불러오기위한준비만 시행, 전체크기를 파악해 저장해둠
    - getitem 에서 인덱스에 해당하는 데이터를 파일에서 직접 불러오고 전처리하여 만들어 반환
    - len 에서는 마찬가지로 저장해둔 전체 길이 출력
  - 생성된 데이터를 DataLoader를 통해 가져오게됨
  - 참고로 torchvision에 들어있는 데이터셋은 대부분 이방식으로 만들어져 있음
